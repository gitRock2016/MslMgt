...
//			 + " where training_record_id = #{id}")
	@Select("select training_record_id,training_start_time,training_events_id  from mslmgt.training_records"
			 + " where training_record_id = #{id}")
	TrgRecordEntity findSampleById(@Param("id") Integer trgRecordId);
	
	@Select("select count(training_record_id) from mslmgt.training_records")
	Integer count();
